...
import { vulnerabilities, assets } from "@/lib/mock-data";
import {
  Share2, Wrench, Shield, Database, AlertTriangle, BookOpen, ExternalLink,
  Monitor, Calendar, Clock, Sparkles, CheckCircle2, Eye, Search,
} from "lucide-react";
...
export default function CveDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const impactedAtms = assets.filter(a => a.type === "ATM" || a.type === "GAB").slice(0, 5);
  ...
  return (
    <div>
      <PageHeader
        breadcrumbs={[{ label: "Vulnerabilities", href: "/vulnerabilities" }, { label: vuln.cveId }]}
        title=""
        actions={
...
{`Deploy-Update -KB "KB5004945" -TargetCollection "ATM-Win10-Fleet" -ForceReboot $true`}
            ...
            {vuln.compensatingControls.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
